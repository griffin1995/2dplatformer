...
class Sky:
    def __init__(self, horizon, style='level'):
        self.top = pygame.image.load(
            '..//game/graphics/decoration/sky/sky_top.png').convert()
        self.middle = pygame.image.load(
            '..//game/graphics/decoration/sky/sky_middle.png').convert()
        self.bottom = pygame.image.load(
            '..//game/graphics/decoration/sky/sky_bottom.png').convert()
        ...
        if self.style == 'overworld':
            palm_surfaces = import_folder(
                '..//game/graphics/overworld/palms')
            self.palms = []
            ...
            cloud_surfaces = import_folder(
                '..//game/graphics/overworld/clouds')
            self.clouds = []
...
class Water:
    def __init__(self, top, level_width):
        ...
        tile_x_amount = int(
            ((level_width + screen_width) / water_tile_width)+4)
        ...
        for tile in range(tile_x_amount):
            ...
            y = top
            sprite = AnimatedTile(
                192, x, y, '..//game/graphics/decoration/water')
...
class Clouds:
    def __init__(self, horizon, level_width, cloud_number):
        cloud_surf_list = import_folder(
            '..//game/graphics/decoration/clouds')
        min_x = -screen_width
